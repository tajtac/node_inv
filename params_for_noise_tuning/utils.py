...
def nn_fpass(H, params):
    ...
    return H
def siren_fpass(H, params, omega_0=10):
    Ws, bs = params
    N_layers = len(Ws)
    
    H = jnp.sin(omega_0 * (jnp.matmul(H, Ws[0]) + bs[0]))
    
    for i in range(1, N_layers - 1):
        H = jnp.sin(jnp.matmul(H, Ws[i]) + bs[i])
...
def init_params_nn(layers, key):
  ...
  return [Ws, bs]
def init_params_siren(layers, key, omega_0=30.0):
    Ws = []
    bs = []
    ...
    for i in range(len(layers) - 1):
        ...
        key, subkey = random.split(key)
        bound = jnp.sqrt(6 / (n))
        W = random.uniform(subkey, (layers[i], layers[i + 1]), minval=-bound, maxval=bound)
